...
from plot_metrics import save_epoch_metrics_figure
...
def main():
    ...
    # Metrics logging
    # =========================================================================
    METRICS_CSV_PATH = "train_metrics_cnextunet_delta.csv"
    ...
    # =========================================================================
    # Model  –  CNextU-Net (delta prediction)
    # =========================================================================
    model = CNextUNet(
    ...
    # =========================================================================
    # Delta-prediction rollout helper
    #   model(x) predicts the *residual* from the last input frame to the
    #   next frame:  pred_{t+1} = x[:, -1] + model(x)
    # =========================================================================
    def rollout_predict_delta(
        model: nn.Module,
        ...
        for _ in range(rollout_steps):
            last_frame = x_roll[:, -1]                         # B H W F
            x_in = rearrange(x_roll, "B Ti H W F -> B (Ti F) H W")
            delta = model(x_in)
            delta = rearrange(delta, "B (To F) H W -> B To H W F",
                              To=1, F=n_fields)[:, 0]         # B H W F
            pred_next = last_frame + delta
    ...
    def compute_train_losses(batch: dict):
        ...
        pred_seq = rollout_predict_delta(model, x, N_STEPS_OUTPUT, N_FIELDS)
    ...
    def evaluate_valid_1step_vrmse(model: nn.Module, loader: DataLoader):
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc="valid 1-step", leave=False):
                x = batch["input_fields"].to(device)
                y = batch["output_fields"].to(device)
                last_frame = x[:, -1]                          # B H W F
                x_in = rearrange(x, "B Ti H W F -> B (Ti F) H W")
                delta = model(x_in)
                delta = rearrange(delta, "B (To F) H W -> B To H W F", To=1, F=N_FIELDS)
                pred = last_frame.unsqueeze(1) + delta         # B 1 H W F
    ...
    def evaluate_valid_rollout_vrmse(model: nn.Module, loader: DataLoader,
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc="valid rollout", leave=False):
                x = batch["input_fields"].to(device)
                y = batch["output_fields"].to(device)
                pred_seq = rollout_predict_delta(model, x, rollout_steps, N_FIELDS)
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        save_epoch_metrics_figure(METRICS_CSV_PATH, epoch, "cnextunet_delta_metrics")
        ...
        if valid_1step["mean_per_field"] < best_valid_1step:
            ...
            torch.save(ckpt_state, "best_cnextunet_delta_by_valid_1step_vrmse.pt")
            print("Saved best_cnextunet_delta_by_valid_1step_vrmse.pt")
        ...
        if valid_rollout["mean_per_field"] < best_valid_rollout:
            ...
            torch.save(ckpt_state, "best_cnextunet_delta_by_valid_rollout_vrmse.pt")
            print("Saved best_cnextunet_delta_by_valid_rollout_vrmse.pt")

    torch.save(ckpt_state, "final_cnextunet_delta.pt")
