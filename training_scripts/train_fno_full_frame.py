...
# metrics logging
# -------------------------
METRICS_CSV_PATH = "train_metrics.csv"
...
# -------------------------
# train dataset: normalized values
# -------------------------
...
# -------------------------
# valid datasets: normalized full-state targets
# -------------------------
...
# -------------------------
# rollout forward for training/eval
# model predicts FULL normalized next state
# -------------------------
def rollout_predict_full(
    ...
    for _ in range(rollout_steps):
        ...
        pred_next_norm = model(x_in)
        pred_next_norm = rearrange(
            pred_next_norm,
        ...
        preds.append(pred_next_norm)
        x_roll = torch.cat([x_roll[:, 1:], pred_next_norm.unsqueeze(1)], dim=1)

    return torch.stack(preds, dim=1)  # (B,T,H,W,F)

# -------------------------
# train losses
# train data is already normalized by dataset
# -------------------------
def compute_train_losses(batch: dict):
    x = batch["input_fields"].to(device)   # normalized full
    y = batch["output_fields"].to(device)  # normalized full

    pred_seq_norm = rollout_predict_full(
        model=model,
        x_init_norm=x,
...
# -------------------------
# aggregated VRMSE helper
# full-state prediction against full-state target
# -------------------------
def aggregated_vrmse_from_stream(pred_seq: torch.Tensor, y: torch.Tensor):
    # pred_seq, y: (B,T,H,W,F)
    sum_err2 = ((pred_seq - y) ** 2).sum(dim=(0, 1, 2, 3))
    sum_y = y.sum(dim=(0, 1, 2, 3))
    sum_y2 = (y * y).sum(dim=(0, 1, 2, 3))
    count = y.shape[0] * y.shape[1] * y.shape[2] * y.shape[3]

    return sum_err2, sum_y, sum_y2, count

# -------------------------
# valid 1-step aggregated VRMSE
...
# -------------------------
# valid rollout aggregated VRMSE
# FULL-STATE rollout evaluation
# -------------------------
...
# -------------------------
# training loop
# choose best model by VALID 1-step VRMSE
# optionally also save best rollout model
# -------------------------
...
for epoch in range(1, EPOCHS + 1):
    ...
    if valid_1step["mean_per_field"] < best_valid_1step:
        ...
        torch.save(model.state_dict(), "best_by_valid_1step_vrmse.pt")
        print("Saved best_by_valid_1step_vrmse.pt")

    if valid_rollout["mean_per_field"] < best_valid_rollout:
        best_valid_rollout = valid_rollout["mean_per_field"]
        torch.save(model.state_dict(), "best_by_valid_rollout_vrmse.pt")
        print("Saved best_by_valid_rollout_vrmse.pt")
torch.save(model.state_dict(), "final_model_full_frame.pt")