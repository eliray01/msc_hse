...
- Текущие лучшие решения: `Delta_64_4` по `1-step`, `Delta_32_4` по `4-step rollout`
...
После добавления новой модели `Delta_64_4` картина результатов стала такой:

- лучший `1-step` результат показывает `Delta_64_4`: `VRMSE = 0.2799` против `0.5121` у `Authors (FNO)`
- лучший `4-step rollout` по `VRMSE` сохраняет `Delta_32_4`: `VRMSE = 0.3411` против `1.6885` у `Authors (FNO)`
- `Delta_64_4` при этом показывает лучший `MAE` на rollout: `0.1269`
...
Среди `full-frame` моделей лучший результат на текущий момент по-прежнему показывает `Full_64_4`, но ветка `delta` остается сильнее как по `1-step`, так и по rollout. Это делает `Delta_32_4` и `Delta_64_4` основной точкой сравнения перед переходом к `U-Net`.
...
| Authors (FNO) | Delta_32_4 | Delta_64_4 | Best full-frame model: Full_64_4 |
| --- | --- | --- | --- |
| ![](plot_gifs_out/plots_delta_gifs/authors_model/traj000_density_pretrained_delta/sequence.gif) | ![](plot_gifs_out/plots_delta_gifs/delta_32_4/traj000_density_delta_last_delta_model/sequence.gif) | ![](plot_gifs_out/plots_delta_gifs/delta_64_4/traj000_density_delta_final_model_delta/sequence.gif) | ![](plot_gifs_out/plots_full_gifs/full_64_4/traj000_density_full_final_model_full_frame/sequence.gif) |
...
- `plot_gifs_out/plots_delta_gifs/delta_64_4/traj000_density_delta_final_model_delta/sequence.gif`
...
Текущие эксперименты показывают, что уже на этапе чекпоинта 5 удалось существенно улучшить baseline и внешний ориентир `Authors (FNO)` по всем ключевым метрикам. После добавления новой модели лидерство разделилось между двумя `delta`-конфигурациями:

- `Delta_64_4` показывает лучший `1-step` результат
- `Delta_32_4` дает лучший `4-step rollout` по `VRMSE`
- обе модели накапливают ошибку заметно медленнее baseline и всех `full-frame` конфигураций

Таким образом, текущая версия проекта уже содержит улучшенное решение и его сравнение с baseline. Следующий исследовательский шаг - проверить, сможет ли `U-Net` превзойти лучшие `FNO`-варианты `Delta_32_4` и `Delta_64_4` на тех же метриках и GIF-сценариях сравнения.
